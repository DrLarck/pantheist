...
async def Wait_for_kick(client, ctx, server, caller, target):
    '''
    ...
    try:
        ...
        await ctx.send(_('<@{}> *[AUTO-KICK]* Aborting.').format(caller.id), delete_after=5)
        ...
        if(message_content == yes_ans):
            try:
                ...
                await server.kick(target, reason='[AUTO-KICK] Kicked after {} warns by {}.'.format(user_warns, caller))
                return
        ...
        if(message_content == no_ans):
            await ctx.send(_('<@{}> *[AUTO-KICK]* Cancellation.').format(caller.id))
            return

async def Wait_for_ban(client, ctx, server, caller, target):
    '''
    ...
    try:
        ...
        await ctx.send(_('<@{}> *[AUTO-BAN]* Aborting.').format(caller.id), delete_after=5)
        ...
        if(message_content == yes_ans):
            try:
                ...
                await server.ban(target, reason='[AUTO-BAN] Banned after {} warns by {}.'.format(user_warns, caller))
        ...
        if(message_content == no_ans):
            await ctx.send(_('<@{}> *[AUTO-BAN]* Cancellation.').format(caller.id))
            return